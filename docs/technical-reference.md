...
- [五bis、GraphQL 数据层（Apollo iOS）](#五bisgraphql-数据层apollo-ios)
...
```swift
// 以下为简化版，省略了导航栏外观配置。完整代码见 NewsApp/AppDelegate.swift
@main
...
这里的关键是通过 `UIWindowScene` API 获取当前窗口的 `UINavigationController`，然后推入一个承载 RN 视图的 `RNViewController`。`props` 参数会作为 React 组件的 `props` 传入，例如将 `category` 传给 NewsList。
...
- `init(moduleName:initialProperties:)`：自定义初始化器，接收 RN 模块名和可选的初始参数（如分类信息）
...
- `initialProperties`：传递给 RN 组件的 props，例如 `["category": "tech"]` 会变成组件的 `{category: "tech"}` prop
...
**文件：** `LocalPods/RNViewFactory/NavigationBridge.m`

**用途：** 允许 RN 的 NewsList 页面触发原生导航操作（推入 NewsDetail 页面）。

```objc
// 完整代码见 LocalPods/RNViewFactory/NavigationBridge.m（此处省略了 topViewController 辅助方法）
...
## 五bis、GraphQL 数据层（Apollo iOS）

### 5b.1 架构设计

新闻数据通过 **Apollo iOS** 在原生侧获取，再经 `NewsDataBridge` 桥接给 RN。这遵循 Brownfield 混合应用的推荐模式——**原生持有网络层**：

```
NewsList.tsx (RN)
  │ NewsDataBridge.fetchNewsFeed("tech")  [Promise]
  ▼
NewsDataBridge.swift (Native Module)
  │ Apollo Client.fetch(NewsFeedQuery(category: "tech"))
  ▼
ApolloService → MockNewsTransport / HTTP Transport
  │ 解析 GraphQL 响应 → 强类型 NewsArticle
  ▼
返回 [NewsItem] 字典数组给 RN
```

**为什么原生持有网络层而非 RN 侧用 `@apollo/client`？**

| 考量 | 说明 |
|---|---|
| **避免缓存不一致** | 两个 Apollo 客户端（Swift + JS）有独立的 normalized cache，数据会 diverge |
| **统一鉴权** | Token 刷新、证书固定、请求拦截器只需在原生侧实现一次 |
| **原生页面可直接用** | 如果 SwiftUI 首页未来需要展示数据，原生侧可直接调用 Apollo |
| **社区共识** | 这是 Brownfield 混合应用最主流的架构模式 |

### 5b.2 GraphQL Schema

**文件：** `NewsApp/GraphQL/schema.graphqls`

```graphql
type Query {
  newsFeed(category: String!, limit: Int = 20): [NewsArticle!]!
  article(id: ID!): NewsArticle
}
...
Apollo iOS 的核心价值是**编译时类型安全**。通过 `apollo-ios-cli` 从 Schema + 查询文件生成 Swift 类型：

```bash
# 安装 CLI（从 GitHub Releases 下载）
curl -sL https://github.com/apollographql/apollo-ios/releases/download/1.25.2/apollo-ios-cli.tar.gz | tar xz

# 生成代码（根目录执行，读取 apollo-codegen-config.json）
./apollo-ios-cli generate
```

**生成的文件（`NewsApp/GraphQL/Generated/`）：**

| 文件 | 作用 |
|---|---|
| `NewsAPI.graphql.swift` | 命名空间枚举 `enum NewsAPI {}` |
| `Schema/SchemaMetadata.graphql.swift` | Schema 元数据（类型映射） |
| `Schema/SchemaConfiguration.swift` | 缓存键配置（可自定义） |
| `Schema/Objects/NewsArticle.graphql.swift` | `NewsArticle` 对象类型定义 |
| `Schema/Objects/Query.graphql.swift` | `Query` 根类型定义 |
| `Schema/CustomScalars/ID.swift` | GraphQL `ID` 标量类型映射 |
| `Operations/Queries/NewsFeedQuery.graphql.swift` | `NewsFeedQuery` 查询类型（强类型 variables 和 Data） |
| `Operations/Queries/ArticleQuery.graphql.swift` | `ArticleQuery` 查询类型 |

**codegen 配置（`apollo-codegen-config.json`）：**

```json
{
  "schemaNamespace": "NewsAPI",
  "input": {
    "operationSearchPaths": ["NewsApp/GraphQL/Operations/**/*.graphql"],
    "schemaSearchPaths": ["NewsApp/GraphQL/schema.graphqls"]
  },
  "output": {
    "testMocks": { "none": {} },
    "schemaTypes": {
      "path": "NewsApp/GraphQL/Generated",
      "moduleType": { "embeddedInTarget": { "name": "NewsApp" } }
    },
...
`embeddedInTarget` 表示生成的代码编译在 `NewsApp` target 中（而非独立模块），所有类型在 `NewsAPI` 命名空间下。

> **CocoaPods 兼容注意：** CocoaPods 将 Apollo 和 ApolloAPI 打包在同一个 `Apollo` 模块中，但 codegen 生成的代码默认 `import ApolloAPI`。生成后需要将 `import ApolloAPI` 替换为 `import Apollo`，将 `ApolloAPI.` 前缀替换为 `Apollo.`。

### 5b.4 Apollo 客户端配置

**文件：** `NewsApp/Services/ApolloService.swift`
...
final class ApolloService {
    ...
    static let useMockData = true  // 改为 false 连接真实服务器
...
**Mock 模式** 使用自定义的 `MockNewsTransport`（实现 `NetworkTransport` 协议），根据操作名返回预定义的 JSON：
...
final class MockNewsTransport: NetworkTransport {
    func send<Operation: GraphQLOperation>(
        ...
        // 根据 Operation.operationName 返回对应的 Mock JSON
        // 通过 JSONSerialization → GraphQLResponse.parseResult() 解析为强类型
    }
}
```

Mock 数据定义在 `NewsApp/Services/MockNewsData.swift`，包含 7 篇新闻文章的完整 GraphQL 响应 JSON。

### 5b.5 NewsDataBridge — 数据桥接

**文件：** `NewsApp/NewsDataBridge.swift` + `NewsApp/NewsDataBridge.m`

这是一个 **Swift 实现 + ObjC 注册** 的混合 Native Module，暴露 Promise 接口给 RN：
    ...
    @objc func fetchNewsFeed(
        ...
        ApolloService.shared.client.fetch(query: query, cachePolicy: .fetchIgnoringCacheCompletely) { result in
            ...
            case .success(let graphQLResult):
                // 将强类型 [NewsFeed] 转为 RN 可用的字典数组
                let items = graphQLResult.data?.newsFeed.map { article in
...
**为什么用 Swift + ObjC 混合而非纯 ObjC？**

Apollo iOS 是 Swift 库，生成的查询类型也是 Swift。在 Swift 中调用 Apollo API 是最自然的。`RCT_EXTERN_MODULE` 宏允许 ObjC 注册一个 Swift 实现的模块。
...
const loadNews = async () => {
    ...
    setNews(data);  // data: [{id, title, summary, body, image, source, time}, ...]
};
```
...
NewsList 通过 `NewsDataBridge` 异步获取数据，包含 loading 和 error/retry 状态：
...
### 10.4 Apollo iOS 1.25.2 — GraphQL 客户端
...
**功能：**
- 强类型 GraphQL 查询（编译时校验字段名和类型）
- 规范化缓存（Normalized Cache），自动去重和更新
- 代码生成（`apollo-ios-cli`），从 `.graphql` 文件生成 Swift 类型
- 请求链拦截器（Request Chain Interceptor）支持自定义认证、重试、日志
- 支持 GraphQL Subscriptions（WebSocket）

**本项目的用法：**

```swift
// 获取新闻列表
let query = NewsAPI.NewsFeedQuery(category: "tech")
ApolloService.shared.client.fetch(query: query) { result in
    switch result {
    case .success(let graphQLResult):
        let articles = graphQLResult.data?.newsFeed  // [NewsFeed] 强类型
    case .failure(let error):
        print(error)
    }
}
...
| 模式 | `ApolloService.useMockData` | 数据来源 |
|---|---|---|
| Mock（当前） | `true` | `MockNewsTransport` → `MockNewsData.swift` 本地 JSON |
| 生产 | `false` | `ApolloClient(url:)` → 真实 GraphQL 服务器 |

### 10.5 Alamofire 5.9 — HTTP 网络请求
...
### 14.1 NewsApp-Bridging-Header.h
...
数据通过 Apollo iOS 的 `NewsFeedQuery` 获取。当前使用 Mock 模式（`MockNewsData.swift` 中 7 条模拟数据）。切换到生产模式后，将从真实 GraphQL 服务器获取。
...
| `NewsAppApp.swift` | Swift | 原始 SwiftUI 入口文件（已废弃）。入口已迁移到 AppDelegate.swift 以支持 RN，此文件仅保留注释说明 |
...
| `NewsDataBridge.swift` | Swift | RN Native Module。通过 Apollo iOS 获取新闻数据，暴露 `fetchNewsFeed`/`fetchArticle` Promise 接口给 JS |
| `NewsDataBridge.m` | ObjC | NewsDataBridge 的 ObjC 注册层（`RCT_EXTERN_MODULE`） |
| `Services/ApolloService.swift` | Swift | Apollo 客户端单例。支持 Mock 模式（本地 JSON）和生产模式（真实 GraphQL 服务器） |
| `Services/MockNewsData.swift` | Swift | Mock 数据层。模拟 GraphQL 服务器返回的 JSON 响应（7 篇新闻） |
| `GraphQL/schema.graphqls` | GraphQL | GraphQL Schema 定义（NewsArticle 类型、newsFeed/article 查询） |
| `GraphQL/Operations/*.graphql` | GraphQL | GraphQL 查询文件（NewsFeedQuery、ArticleQuery） |
| `GraphQL/Generated/` | Swift | apollo-ios-cli 自动生成的强类型 Swift 代码（命名空间 NewsAPI） |
| `NewsApp-Bridging-Header.h` | ObjC | Swift ↔ ObjC 桥接头文件。暴露 RCT 和 RNViewFactory 头文件给 Swift |
...
> **注意：** `NavigationBridge` 的实现位于 `LocalPods/RNViewFactory/NavigationBridge.m`（Pod 内部），`NewsApp/` 目录下不再有副本。如需修改 NavigationBridge 的逻辑，请编辑 Pod 内的文件。

### 配置文件（根目录）

| 文件 | 职责 |
|---|---|
| `NewsApp.xcworkspace/` | **Xcode 工作区（开发时必须打开这个）**。由 CocoaPods 生成，整合主工程和 Pods 依赖 |
| `NewsApp.xcodeproj/` | Xcode 项目文件。包含编译设置、文件引用等。**不要单独打开此文件**，否则缺少 Pod 依赖 |
...
| `apollo-codegen-config.json` | Apollo iOS 代码生成配置。指定 Schema 路径、查询文件路径、输出目录和模块类型 |
| `app.json` | RN 应用名称配置（`{"name": "NewsApp", "displayName": "NewsApp"}`） |
...
| `README.md` | 英文项目文档（技术栈、项目结构、架构说明、快速开始等） |
| `README_CN.md` | 中文项目文档（与 README.md 内容一致的中文翻译） |
...
| 修改 Swift / ObjC 文件（`NewsApp/` 目录） | **Xcode 重新编译（Cmd+R）** |
...
| 修改 GraphQL Schema 或查询 | `apollo-ios-cli generate` → Xcode 重新编译 |

### 18.3 修改原生代码

修改 `NewsApp/` 目录下的 Swift 或 ObjC 文件后：
...
1. 编辑 `Podfile`，在 `target 'NewsApp' do` 内添加新 Pod：
...
新建文件 `NewsApp/ClipboardBridge.m`：
...
> **注意：** 新建 `.m` 文件后需要在 Xcode 中确认它被添加到了 `NewsApp` target 的 Compile Sources 中。通常 Xcode 会自动处理，但如果运行时 JS 报 "Module not found"，请手动检查：Xcode → NewsApp target → Build Phases → Compile Sources。
